      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 py-4 sm:py-6">
        ...
        <div className="flex items-start justify-between mb-4 sm:mb-6 gap-4">
          <div className="min-w-0 flex-1">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Painel de Gestão</h2>
            <p className="text-sm sm:text-base text-gray-600 hidden sm:block">Gerencie todos os pedidos de materiais</p>
          </div>
          <Button 
            onClick={() => setActiveTab('users')}
            variant="outline"
            className="flex items-center space-x-1 sm:space-x-2 text-sm px-3 sm:px-4"
          >
            <Users className="w-4 h-4" />
            <span className="hidden sm:inline">Aprovar Usuários</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-4 mb-4 sm:mb-6">
          ...
          <Card className="p-3 sm:p-4">
            <div className="flex items-center space-x-2 sm:space-x-3">
              ...
              <div className="min-w-0">
                <p className="text-lg sm:text-2xl font-bold text-gray-800">{stats.myOrdersCount}</p>
                <p className="text-xs sm:text-sm text-gray-600">Meus</p>
              </div>
            </div>
          </Card>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 sm:space-y-6">
          <TabsList className="grid w-full grid-cols-3 h-auto">
            <TabsTrigger value="available" className="text-xs sm:text-sm px-2 sm:px-4 py-2">
              <span className="hidden sm:inline">Disponíveis ({availableOrders.length})</span>
              <span className="sm:hidden">Disp. ({availableOrders.length})</span>
            </TabsTrigger>
            <TabsTrigger value="mine" className="text-xs sm:text-sm px-2 sm:px-4 py-2">
              <span className="hidden sm:inline">Meus Pedidos ({myOrders.length})</span>
              <span className="sm:hidden">Meus ({myOrders.length})</span>
            </TabsTrigger>
            <TabsTrigger value="all" className="text-xs sm:text-sm px-2 sm:px-4 py-2">
              <Eye className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" />
              <span className="hidden sm:inline">Painel Geral ({orders.length})</span>
              <span className="sm:hidden">Geral ({orders.length})</span>
            </TabsTrigger>
          </TabsList>
